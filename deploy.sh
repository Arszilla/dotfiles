...
do
    ...
    then
        ...
        then
            ...
            cp -r ./"Laptop Specific"/etc/skel/. /home/$SUDO_USER/
            
        elif [ "$DEVICE" -eq 2 ]
        then
            # Move the laptop specific files to /home/$SUDO_USER/:
            cp -r ./"Desktop Specific"/etc/skel/. /home/$SUDO_USER/
...
sudo -H -u $SUDO_USER sh -c "$(curl -fsSL https://raw.githubusercontent.com/ohmyzsh/ohmyzsh/master/tools/install.sh)" "" --unattended

# oh-my-zsh autosuggestions plugin installation:
sudo -H -u $SUDO_USER git clone https://github.com/zsh-users/zsh-autosuggestions ${ZSH_CUSTOM:-/home/$SUDO_USER/.oh-my-zsh/custom}/plugins/zsh-autosuggestions

# oh-my-zsh completions plugin installation:
sudo -H -u $SUDO_USER git clone https://github.com/zsh-users/zsh-completions ${ZSH_CUSTOM:-/home/$SUDO_USER/.oh-my-zsh/custom}/plugins/zsh-completions

# Add the dotfiles to the /home/$SUDO_USER/:
cp -r ./etc/skel/. /home/$SUDO_USER/

# Replace SSH's config at /etc/ssh/:
cp -r ./etc/ssh/ssh_config /etc/ssh/
